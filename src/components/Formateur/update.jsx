...
export default ({
  ...
  const [nomInput, setNomInput] = useState(itemToEdit.name);
  const [cinInput, setCinInput] = useState(itemToEdit.cin);
  const [emailInput, setEmailInput] = useState(itemToEdit.email);
  const [filiereInput, setFiliereInput] = useState(itemToEdit.filieres);
  const [groupeInput, setGroupeInput] = useState(itemToEdit.groupes);
  const [groupesIdsInput, setGroupesIdsInput] = useState(itemToEdit.groupes);

  const handleFiliereSelection = (e) => {
    const filieres = Array.from(e.target.selectedOptions, (option) =>
      option.getAttribute("data-filiere")
    );
    const groupes = Array.from(
      e.target.selectedOptions,
      (option) => option.value
    );
    setFiliereInput([...new Set(filieres)]);
    setGroupeInput(groupes);
  };

  useEffect(() => {
    if (firstLoad) {
      const groupesIds = itemToEdit?.groupes?.flatMap((item) => {
        return item.id;
      });
      console.log(itemToEdit);
      setGroupesIdsInput(groupesIds);
      setFirstLoad(false);
    }
  }, []);
            ...
            <div class="relative p-4 bg-white rounded-lg shadow sm:p-5">
              ...
              <form action="#" enctype="multipart/form-data">
                <div class="flex flex-col gap-4">
                  <div className="wrrap w-full flex gap-4">
                    <div className="wrap w-full flex flex-col gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          id="brand"
                          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg   block w-full p-2.5"
                          placeholder="Adress e-mail de formateur"
                          required=""
                        />
                      </div>
                    </div>
                    <div className="wrrap w-full flex flex-col gap-2">
                      <div className="h-full">
                        ...
                        <select
                          ...
                          id="filiere"
                          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg   block w-full p-2.5 min-h-full">
                          {filieres?.map((filiere, index) => {
                            console.log(groupesIdsInput);
                                ...
                                {filiere?.groupes?.map((groupe) => {
                                  return (
                                    <option
                                      ...
                                      selected={groupesIdsInput.includes(
                                        groupe.id
                                      )}
                  ...
                  <div className="CTAs flex gap-2 items-center justify-start">
                    <button
                      type="button"
